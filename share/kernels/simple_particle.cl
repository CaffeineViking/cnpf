void kernel simple_particle(global float* positions, global float* velocities,read_only image2d_t texture, const float time)
{
    int id = get_global_id(0);
    const sampler_t smp = CLK_NORMALIZED_COORDS_FALSE | //Natural coordinates
	CLK_ADDRESS_CLAMP | //Clamp to zeros
	CLK_FILTER_LINEAR; //Don't interpolate
	float4 values = read_imagef(texture, smp, (int2)(positions[3*id+0],positions[3*id+2]));
	velocities[3*id+0] = values.x * 0.001f;
	velocities[3*id+1] = values.y* 0.001f;
	velocities[3*id+2] = values.z* 0.001f;
