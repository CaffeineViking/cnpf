							...
							const float time)
{
	...
	velocities[3*id+0] = (values.x - 0.5f) * 0.02f;
	velocities[3*id+1] = (values.y - 0.5f) * 0.02f;
	velocities[3*id+2] = (values.z - 0.5f) * 0.02f;
   	positions[3*id+0] += velocities[3*id+0];
	positions[3*id+1] += velocities[3*id+1];
    positions[3*id+2] += velocities[3*id+2];
}