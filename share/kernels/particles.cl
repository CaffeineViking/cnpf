							...
							const float frameDelta)
{
	...
	float4 values = read_imagef(texture, smp, (float4)(x/width,y/height,z/depth,0.0f));
   	positions[3*id+0] += (values.x - 0.5f) * 2.0f * frameDelta  * velocities[3*id+0];
	positions[3*id+1] += (values.y - 0.5f) * 2.0f * frameDelta  * velocities[3*id+1];
    positions[3*id+2] += (values.z - 0.5f) * 2.0f * frameDelta  * velocities[3*id+2];
}